/*!\addtogroup ManikinOS
  \file CompressionController.ino
  \ingroup ManikinOS
  @{
  @brief This is the main file of the compression controller of the ManikinOS.
  ...
  while (1){
    ...
    if ( time_slot_notification  > 0) {
      ...
      {  
        ...
        if (incomingByte == 'd'){
          Serial.println("DELAY computeResult!");
          myDelayMs(1500);
        }
        /// Task job finished, increment the time slot semaphore
...
void setup()
{
  ...
  if (show_debug) {Serial.println("******************************");}
  if (show_debug) {Serial.println("       Program CC start       ");}
  if (show_debug) {Serial.println("******************************");}
...
{  
  ...
  if ( start_time_slot_schedule == false) {
    ...
    if (global_time.millisecond % TIME_SLOT_DURATION_MS == 0.0) {
